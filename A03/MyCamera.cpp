...
// Calculates movement forward relative to the camera
void MyCamera::MoveForward(float a_fDistance)
{
	...
	m_v3Above = m_v3Position + vector3(0.0f, 1.0f, 0.0f);
}

// Uses the camera's global position to move it vertically.
...
// Uses a cross product of the Target and Above vectors to calculate sideways movement relative to the camera.
void MyCamera::MoveSideways(float a_fDistance)
{
	m_v3Target += glm::normalize(glm::cross(m_v3Above - m_v3Position, m_v3Target - m_v3Position)) * (a_fDistance);
	m_v3Position += glm::normalize(glm::cross(m_v3Above - m_v3Position, m_v3Target - m_v3Position)) * (a_fDistance);
	m_v3Above = m_v3Position + vector3(0.0f, 1.0f, 0.0f);
}