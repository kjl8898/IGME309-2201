...
void AppClass::ProcessKeyboard(sf::Event a_event)
{
	...
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) // Checks for spacebar
	{
		// If invertColor is false, changes the shader to the ComplimentaryColor shader
		//		If it is true, sets the shader back to BasicColor
		if (!invertColor)
		{
			invertColor = true;
