...
void Application::InitVariables(void)
{
	...
	m_sProgrammer = "Kevin Laporte - kjl8898@rit.edu";
...
void Application::Display(void)
{
	...
	/// <summary>
	/// DOES NOT WORK PAST 2 CYCLES
	///		I struggled to use the timer to help determine the proper index in the vector.
	///		The program works and demostrates my understanding of the LERP function, however has
	///		in index out of range after 2 cycles. I am unsure of how to properly fix this, but figured
	///		I'd submit it anyway since I spent a long time trying to work this all out.
	/// </summary>

	static int cycles = 0;
	static bool cycled = false;
	int currentSec = fTimer;

	// Checks if it is on the last vertex in the vector
	if (currentSec - (cycles * (m_stopsList.size() - 1)) == m_stopsList.size() - 1)
	{
		v3CurrentPos = glm::lerp(m_stopsList[m_stopsList.size() - 1], m_stopsList[0], fTimer - currentSec);
	...
	else
	{
		...
		int a = currentSec - (cycles * (m_stopsList.size()));
		int b = currentSec + 1 - (cycles * (m_stopsList.size()));

		v3CurrentPos = glm::lerp(m_stopsList[a], m_stopsList[b], fTimer - currentSec);
	}
